class ChangeVisitLocationIdColumn < ActiveRecord::Migration[7.0]
  def change
    # Remove the existing foreign key in visits table if it exists
    if foreign_key_exists?(:visits, :locations, column: :location_id)
      remove_foreign_key :visits, :locations
    end

    # Ensure location_id column exists and is of the correct type (string, varchar(255))
    unless column_exists?(:visits, :location_id)
      add_column :visits, :location_id, :string, limit: 255, null: true
    else
      # Change column type to string to match updated requirements
      change_column :visits, :location_id, :string, limit: 255, null: true
    end
