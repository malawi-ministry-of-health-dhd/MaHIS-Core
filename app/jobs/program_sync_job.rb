# app/jobs/program_sync_job.rb
class ProgramSyncJob
  include Sidekiq::Job
  include CouchdbSync
  ...
  def perform(batch_size = 100) # Reduced default batch size
    ...
    total_count = Program.where(retired: 0).count
    Sidekiq.logger.info "Starting sync of #{total_count} programs to CouchDB at #{COUCHDB_URL}"
    
    processed = 0
    errors = []
    consecutive_errors = 0
    
    Program.where(retired: 0).find_in_batches(batch_size: batch_size) do |program_batch|
      program_batch.each_with_index do |program, index|
        begin
          sync_program_to_couchdb(program, db_name)
          processed += 1
          ...
          if processed % 100 == 0
            Sidekiq.logger.info "Synced #{processed}/#{total_count} programs"
          end
          
        rescue RestClient::Exception, SocketError, Errno::ECONNREFUSED => e
          consecutive_errors += 1
          error_msg = "Failed to sync program ID #{program.program_id}: #{e.message}"
      ...
      sleep(0.1)
      Sidekiq.logger.info "Completed batch. Processed #{processed}/#{total_count} programs so far."
    end
    ...
    if errors.any?
      ...
      if errors.length > total_count * 0.05 # Fail if >5% error rate
        raise "Program sync completed with unacceptable error rate: #{errors.length}/#{total_count} (#{(errors.length.to_f/total_count*100).round(2)}%)"
      end
    end
  end
  ...
  def check_and_clean_couchdb_if_needed(db_name)
    begin
      mysql_count = Program.where(retired: 0).count
      couchdb_count = get_couchdb_program_count(db_name)
      
      Sidekiq.logger.info "MySQL program count: #{mysql_count}, CouchDB program count: #{couchdb_count}"
      
      if mysql_count != couchdb_count
        Sidekiq.logger.warn "Record count mismatch detected! MySQL: #{mysql_count}, CouchDB: #{couchdb_count}"
        Sidekiq.logger.info "Cleaning all program records from CouchDB before sync..."
        
        delete_all_programs_from_couchdb(db_name)
        
        Sidekiq.logger.info "Successfully cleaned all program records from CouchDB"
  ...
  def get_couchdb_program_count(db_name)
    begin
      ...
      # Get count of program documents specifically using URL encoding
      require 'uri'
      start_key = URI.encode_www_form_component('"program_"')
      end_key = URI.encode_www_form_component('"program_\ufff0"')
      ...
      Sidekiq.logger.error "Error getting CouchDB program count: #{e.message}"
      raise e
    end
  end
  
  def delete_all_programs_from_couchdb(db_name)
    begin
      db_url = "#{COUCHDB_URL}/#{db_name}"
      ...
      # Get all program documents using URL encoding
      require 'uri'
      start_key = URI.encode_www_form_component('"program_"')
      end_key = URI.encode_www_form_component('"program_\ufff0"')
      ...
      if result['rows'].empty?
        Sidekiq.logger.info "No program documents found in CouchDB. Nothing to clean."
        return
      end
      
      Sidekiq.logger.info "Found #{result['rows'].length} program documents to delete"
      ...
      Sidekiq.logger.info "Successfully deleted #{successful_deletes} program documents from CouchDB"
      ...
      Sidekiq.logger.error "Error deleting programs from CouchDB: #{e.message}"
      raise e
    end
  end
  
  def sync_program_to_couchdb(program, db_name)
    doc_data = prepare_program_document(program)
    doc_id = "program_#{program.program_id}"
  ...
  def prepare_program_document(program)
    {
      "type" => "program",
...
# Usage examples:
# ProgramSyncJob.perform_async(50)  # Even smaller batches
# ProgramSyncJob.perform_async     # Default batch size of 100