# app/jobs/specimen_sync_job.rb
class SpecimenSyncJob
  include Sidekiq::Job
  include CouchdbSync
  ...
  def perform(batch_size = 50) # Smaller default batch size since specimens are fewer
    db_name = 'specimens'
    ...
    return if check_and_clean_couchdb_if_needed(db_name, specimen_types) == :skip_sync
    
    total_count = specimen_types.length
    Sidekiq.logger.info "Starting sync of #{total_count} specimen types to CouchDB at #{COUCHDB_URL}"
    ...
    # Process in batches
    specimen_types.each_slice(batch_size).with_index do |specimen_batch, batch_index|
      specimen_batch.each_with_index do |specimen, index|
        begin
          sync_specimen_to_couchdb(specimen, db_name)
          processed += 1
          ...
          # Log progress every 25 records (smaller intervals for specimens)
          if processed % 25 == 0
            Sidekiq.logger.info "Synced #{processed}/#{total_count} specimen types"
          end
          
        rescue RestClient::Exception, SocketError, Errno::ECONNREFUSED => e
          consecutive_errors += 1
          error_msg = "Failed to sync specimen concept ID #{specimen.concept_id}: #{e.message}"
      ...
      sleep(0.1)
      Sidekiq.logger.info "Completed batch #{batch_index + 1}. Processed #{processed}/#{total_count} specimen types so far."
    end
    ...
    if errors.any?
      ...
      if errors.length > total_count * 0.05 # Fail if >5% error rate
        raise "Specimen sync completed with unacceptable error rate: #{errors.length}/#{total_count} (#{(errors.length.to_f/total_count*100).round(2)}%)"
      end
    end
  end
  ...
  def check_and_clean_couchdb_if_needed(db_name, specimen_types)
    begin
      mysql_count = specimen_types.length
      couchdb_count = get_couchdb_specimen_count(db_name)
      
      Sidekiq.logger.info "MySQL specimen count: #{mysql_count}, CouchDB specimen count: #{couchdb_count}"
      
      if mysql_count != couchdb_count
        Sidekiq.logger.warn "Record count mismatch detected! MySQL: #{mysql_count}, CouchDB: #{couchdb_count}"
        Sidekiq.logger.info "Cleaning all specimen records from CouchDB before sync..."
        
        delete_all_specimens_from_couchdb(db_name)
        
        Sidekiq.logger.info "Successfully cleaned all specimen records from CouchDB"
  ...
  def get_couchdb_specimen_count(db_name)
    begin
      ...
      # Get count of specimen documents specifically using URL encoding
      require 'uri'
      start_key = URI.encode_www_form_component('"specimen_"')
      end_key = URI.encode_www_form_component('"specimen_\ufff0"')
      ...
      Sidekiq.logger.error "Error getting CouchDB specimen count: #{e.message}"
      raise e
    end
  end
  
  def delete_all_specimens_from_couchdb(db_name)
    begin
      db_url = "#{COUCHDB_URL}/#{db_name}"
      ...
      # Get all specimen documents using URL encoding
      require 'uri'
      start_key = URI.encode_www_form_component('"specimen_"')
      end_key = URI.encode_www_form_component('"specimen_\ufff0"')
      ...
      if result['rows'].empty?
        Sidekiq.logger.info "No specimen documents found in CouchDB. Nothing to clean."
        return
      end
      
      Sidekiq.logger.info "Found #{result['rows'].length} specimen documents to delete"
      ...
      Sidekiq.logger.info "Successfully deleted #{successful_deletes} specimen documents from CouchDB"
      ...
      Sidekiq.logger.error "Error deleting specimens from CouchDB: #{e.message}"
      raise e
    end
  end
  
  def sync_specimen_to_couchdb(specimen, db_name)
    doc_data = prepare_specimen_document(specimen)
    doc_id = "specimen_#{specimen.concept_id}"
  ...
  def prepare_specimen_document(specimen)
    {
      "type" => "specimen",
...
# Usage examples:
# SpecimenSyncJob.perform_async     # Default batch size of 50
# SpecimenSyncJob.perform_async(25) # Smaller batches
# SpecimenSyncJob.perform_async(10) # Very small batches for careful processing