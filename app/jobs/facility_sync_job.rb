# app/jobs/facility_sync_job.rb
class FacilitySyncJob
  include Sidekiq::Job
  include CouchdbSync
  ...
  def perform(batch_size = 100) # Reduced default batch size
    ...
    total_count = Facility.count
    Sidekiq.logger.info "Starting sync of #{total_count} facilities to CouchDB at #{COUCHDB_URL}"
    
    processed = 0
    errors = []
    consecutive_errors = 0
    
    Facility.find_in_batches(batch_size: batch_size) do |facility_batch|
      facility_batch.each_with_index do |facility, index|
        begin
          sync_facility_to_couchdb(facility, db_name)
          processed += 1
          ...
          if processed % 100 == 0
            Sidekiq.logger.info "Synced #{processed}/#{total_count} facilities"
          end
          
        rescue RestClient::Exception, SocketError, Errno::ECONNREFUSED => e
          consecutive_errors += 1
          error_msg = "Failed to sync facility ID #{facility.id}: #{e.message}"
      ...
      sleep(0.1)
      Sidekiq.logger.info "Completed batch. Processed #{processed}/#{total_count} facilities so far."
    end
    ...
    if errors.any?
      ...
      if errors.length > total_count * 0.05 # Fail if >5% error rate
        raise "Facility sync completed with unacceptable error rate: #{errors.length}/#{total_count} (#{(errors.length.to_f/total_count*100).round(2)}%)"
      end
    end
  end
  ...
  def check_and_clean_couchdb_if_needed(db_name)
    begin
      mysql_count = Facility.count
      couchdb_count = get_couchdb_facility_count(db_name)
      
      Sidekiq.logger.info "MySQL facility count: #{mysql_count}, CouchDB facility count: #{couchdb_count}"
      
      if mysql_count != couchdb_count
        Sidekiq.logger.warn "Record count mismatch detected! MySQL: #{mysql_count}, CouchDB: #{couchdb_count}"
        Sidekiq.logger.info "Cleaning all facility records from CouchDB before sync..."
        
        delete_all_facilities_from_couchdb(db_name)
        
        Sidekiq.logger.info "Successfully cleaned all facility records from CouchDB"
  ...
  def get_couchdb_facility_count(db_name)
    begin
      ...
      # Get count of facility documents specifically using URL encoding
      require 'uri'
      start_key = URI.encode_www_form_component('"facility_"')
      end_key = URI.encode_www_form_component('"facility_\ufff0"')
      ...
      Sidekiq.logger.error "Error getting CouchDB facility count: #{e.message}"
      raise e
    end
  end
  
  def delete_all_facilities_from_couchdb(db_name)
    begin
      db_url = "#{COUCHDB_URL}/#{db_name}"
      ...
      # Get all facility documents using URL encoding
      require 'uri'
      start_key = URI.encode_www_form_component('"facility_"')
      end_key = URI.encode_www_form_component('"facility_\ufff0"')
      ...
      if result['rows'].empty?
        Sidekiq.logger.info "No facility documents found in CouchDB. Nothing to clean."
        return
      end
      
      Sidekiq.logger.info "Found #{result['rows'].length} facility documents to delete"
      ...
      Sidekiq.logger.info "Successfully deleted #{successful_deletes} facility documents from CouchDB"
      ...
      Sidekiq.logger.error "Error deleting facilities from CouchDB: #{e.message}"
      raise e
    end
  end
  
  def sync_facility_to_couchdb(facility, db_name)
    doc_data = prepare_facility_document(facility)
    doc_id = "facility_#{facility.id}"
  ...
  def prepare_facility_document(facility)
    {
      "type" => "facility",
...
# Usage examples:
# FacilitySyncJob.perform_async(50)  # Even smaller batches
# FacilitySyncJob.perform_async     # Default batch size of 100