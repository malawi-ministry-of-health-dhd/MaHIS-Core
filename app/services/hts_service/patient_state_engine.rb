...
module HtsService
  class PatientStateEngine
    ...
    CURRENT_FACILITY = Location.find_by(location_id: GlobalProperty.find_by_property('current_health_center_id')&.property_value&.to_i)&.name || Location.current&.name
    ...
    def link_patient
      ...
      ActiveRecord::Base.transaction do
        ...
        build_art_outcome_obs.collect do |ob|
    ...
    def build_art_outcome_obs
      [
        { concept_id: concept('Antiretroviral status or outcome').concept_id,
          value_coded: concept('Linked').concept_id },
    ...
    def is_new_patient?
      Observation.where(person_id: patient.patient_id,
                        concept: concept('Type of patient')).first.value_coded === 7572
    end
    ...
    def patient_is_not_linked?
      ...
      Logger.debug "Patient #{patient.id} already linked to HTS" if exists
      !exists
    end
