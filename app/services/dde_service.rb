...
class DdeService
  ...
  # for the patient.
  def create_patient(patient, npid)
    push_local_patient_to_dde(patient, npid)
  end
  ...
  # generated by DDE.
  def push_local_patient_to_dde(patient, npid)
    Rails.logger.info("Pushing local patient ##{patient.patient_id} to DDE")
    response, status = dde_client.post('add_person', **openmrs_to_dde_patient(patient, npid))
