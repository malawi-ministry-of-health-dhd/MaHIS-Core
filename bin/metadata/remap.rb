...
def load_diff(table:)
  ...
  sql = format("SELECT uuid FROM %<table>s WHERE uuid NOT IN (SELECT uuid FROM %<master>s.%<table>s)", table:, master: masterdb)
  diff = SlaveBase.connection.select_all(sql)

  log("Found #{diff.count} diff records in #{table}")
  return diff.map { |r| r["uuid"] }
end
...
def update_tx_tables
  ...
  txn_remap_config.each do |table, config|
    config.each do |c|
      ...
      log("Updating table #{table} for column #{column}")
      sql = <<~SQL
        UPDATE %<table>s
        SET %<column>s = (
