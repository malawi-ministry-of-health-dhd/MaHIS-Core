...
describe Report do
  describe 'cascade delete for drill-down records' do
    ...
    before(:each) do
      # Create a minimal user for creator validation, bypassing normal validations
      unless User.find_by(user_id: 1)
        ActiveRecord::Base.connection.execute(
          "INSERT INTO users (user_id, username, creator, system_id, uuid, date_created) 
           VALUES (1, 'test', 1, 'test', UUID(), NOW())"
        )
      end
