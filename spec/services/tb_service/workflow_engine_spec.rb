...
describe TbService::WorkflowEngine do
  ...
  describe :next_encounter do
    ...
    it 'returns LAB RESULTS for a TB suspect after 1 hour prior a Lab Order' do
      enroll_patient patient
      test_procedure_type(patient, tb_initial_encounter(patient, Time.now - 2.hour), 'Laboratory examinations',
                          Time.now - 2.hour)
      lab_orders_encounter(patient, Time.now - 2.hour)
      encounter_type = engine.next_encounter
    ...
    it 'returns TB Adherence for a follow up patient' do
      ...
      tb_initial_encounter(patient, Time.now - 2.day)
      test_procedure_type(patient, tb_initial_encounter(patient, Time.now - 2.day), 'Laboratory examinations',
                          Time.now - 2.day)
  ...
  def tb_initial_encounter(patient, datetime)
    create :encounter, type: encounter_type('TB_INITIAL'),
  ...
  def diagnosis_encounter(patient, datetime)
    create :encounter, type: encounter_type('DIAGNOSIS'),
                       patient:, program_id: tb_program.program_id, encounter_datetime: datetime
  end

  # patient should recieve drugs
  def adherence(patient, datetime)
    create :encounter, type: encounter_type('TB ADHERENCE'),
                       patient:, program_id: tb_program.program_id, encounter_datetime: datetime
  end
  ...
  def create_minor_patient
    ...
    PersonName.create(person_id: person.person_id, given_name: 'John',
                      family_name: 'Doe')
    Patient.create(patient_id: person.person_id)
  end
  ...
  def back_data_tb_initial_encounter(patient, datetime)
    create :encounter, type: encounter_type('TB_INITIAL'),
                       patient:, program_id: tb_program.program_id,
                       encounter_datetime: datetime
  end

  def tb_reception(patient, datetime)
    create :encounter, type: encounter_type('TB RECEPTION'),
                       patient:, program_id: tb_program.program_id,
                       encounter_datetime: datetime
  end
